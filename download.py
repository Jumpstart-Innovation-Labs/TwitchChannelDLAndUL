...
def _video_target_filename(video, format):
    ...
    return "VOD/"+ video['creator']['login'] +'/' + name + "." + format
...
def _download_video(video_id, format='mkv', max_workers=20, quality=None, start=None, end=None, overwrite=False):
    ...
    path_map = download_files(base_uri, target_dir, vod_paths, max_workers)
    ...
    _join_vods(playlist_path, target, overwrite)
